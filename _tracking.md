...
| PV 連結接上（07-17） | ✅ 結案（commit d5d669c；推 main+部署 gh-pages） | 「觀看完整 PV」改外部連結（同下載 Demo 處理），SharePoint 影片；同時清掉不再用的站內 lightbox 骨架（index.html/hero.ts initLightbox/hero.css） | — |
| 繪馬堂過時截圖排查+重截（07-17） | ✅ 結案：親自比對 16 張圖，確認 4 張過時（street_day/dusk/night 帶已刪除舊 MapHUD 日期字卡；dialogue 字級偏小），其餘 12 張核對後正常。過時 4 張派 subagent 用 Godot 實機重截（真實場景背景，非測試灰底），read-back 目視驗收通過（新版左側按鍵提示+主線追蹤框可見、對話字級明顯放大） | public/media/img/gallery/{street_day,street_dusk,street_night,dialogue}[_thumb].avif；新版 Godot 截圖腳本存 MONK/test/CaptureGalleryRefresh.gd(+.tscn)，觸發了塵對話法＝Dialogic.start("main_ch1_intel")+create_timer(1.0)等join跑完+skip_text_reveal() | 舊截圖來源存 website/_art_src/gallery_refresh_20260717/ |
